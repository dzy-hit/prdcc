...
public:
  ...
  void initialize(const Context &context) {
    ...
    // fixme: --mvcc 与缓存计算不能并存
    // 在所有表初始化完成后，立即计算并缓存全量键集合
    LOG(INFO) << "Pre-computing and caching all YCSB keys for DataSummary...";
    this->all_keys_cache_ = collect_all_keys_internal();
    LOG(INFO) << "Key caching complete. Cached " << this->all_keys_cache_.size() << " keys.";
  }
...
private:
  std::vector<uint64_t> collect_all_keys_internal() {
      ...
      // 这个函数体内的逻辑不变，只是现在它被 initialize 调用一次
