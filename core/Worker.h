...
public:
  Worker(std::size_t coordinator_id, std::size_t id)
    ...
    timer_phase1_ns.store(0);
    timer_phase2_ns.store(0);
  }
...
public:
  ...
  // Optional protocol timing (enabled via Context::enable_timer).
  // Aria: phase1=execution, phase2=abort/retry. Prodcc: phase1=prediction,
  // phase2=schedule+execution.
  std::atomic<uint64_t> timer_phase1_ns, timer_phase2_ns;
};
