...
public:
  ...
  std::tuple<MetaDataType *, void *> search_prev(const void *key,
    ...
    // CHECK(v_ptr != nullptr)
    //     << "key with version: " << version << " does not exist.";
    // 如果找不到更早的版本（v_ptr 为 nullptr），则优雅地返回空元组。
    if (v_ptr == nullptr) {
  ...
  void for_each_key(std::function<void(const void *)> f) override {
      // The underlying MVCCHashMap does not currently support iteration.
      // For this implementation, we will assume that for initial data loading,
      // a simple key set is sufficient and the complexity of iterating through
      // version chains is not needed. If MVCCHashMap supported iteration,
      // we would implement it here.
      // For now, let's add a placeholder CHECK.
      CHECK(false) << "for_each_key not implemented for MVCCTable yet.";
  }
