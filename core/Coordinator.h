...
public:
  ...
  void start() {
    ...
    // // timer_flag
    // uint64_t total_timer_phase1_ns = 0, total_timer_phase2_ns = 0;
    int count = 0;
    ...
    do {
      ...
      // // timer_flag
      // uint64_t n_timer_phase1_ns = 0, n_timer_phase2_ns = 0;
      ...
      for (auto i = 0u; i < workers.size(); i++) {
        ...
        //     (context.protocol == "Aria" || context.protocol == "Prodcc")) {
        //   uint64_t p1 = 0, p2 = 0;
        //   workers[i]->drain_timer(p1, p2);
        //   n_timer_phase1_ns += p1;
        //   n_timer_phase2_ns += p2;
        // }
      }

      // // timer_flag
      // const double avg_network_size =
      //     n_commit == 0 ? 0.0 : (1.0 * n_network_size / n_commit);
      // const double si_pct =
      //     n_commit == 0 ? 0.0 : (100.0 * n_si_in_serializable / n_commit);
      // const double local_pct = n_commit == 0 ? 0.0 : (100.0 * n_local / n_commit);

      // std::string timer_suffix;
      // if (context.enable_timer &&
      //     (context.protocol == "Aria" || context.protocol == "Prodcc")) {
      //   const char *phase1 =
      //       context.protocol == "Aria" ? "exec" : "predict";
      //   const char *phase2 =
      //       context.protocol == "Aria" ? "abort_retry" : "schedule_exec";
      //   const double phase1_ms = n_timer_phase1_ns / 1e6;
      //   const double phase2_ms = n_timer_phase2_ns / 1e6;
      //   const double total_ms = phase1_ms + phase2_ms;
      //   const double phase2_pct =
      //       total_ms <= 0.0 ? 0.0 : (100.0 * phase2_ms / total_ms);
      //   std::ostringstream oss;
      //   oss.setf(std::ios::fixed);
      //   oss << std::setprecision(2);
      //   oss << ", timer(" << phase1 << "/" << phase2 << "): " << phase1_ms
      //       << "/" << phase2_ms << " ms, " << phase2_pct << "% in phase2";
      //   timer_suffix = oss.str();
      // }
      ...
      //           << ", avg network size: " << avg_network_size
      //           << ", si_in_serializable: " << n_si_in_serializable << " "
      //           << si_pct << " %"
      //           << ", local: " << local_pct << " %" << timer_suffix;
      ...
      if (count > warmup && count <= timeToRun - cooldown) {
        ...
        // // timer_flag
        // total_timer_phase1_ns += n_timer_phase1_ns;
        // total_timer_phase2_ns += n_timer_phase2_ns;
      }
    ...
    // if (context.enable_timer &&
    //     (context.protocol == "Aria" || context.protocol == "Prodcc")) {
    //   const char *phase1 = context.protocol == "Aria" ? "exec" : "predict";
    //   const char *phase2 =
    //       context.protocol == "Aria" ? "abort_retry" : "schedule_exec";
    //   const double phase1_ms = (total_timer_phase1_ns / 1e6) / count;
    //   const double phase2_ms = (total_timer_phase2_ns / 1e6) / count;
    //   const double total_ms = phase1_ms + phase2_ms;
    //   const double phase2_pct =
    //       total_ms <= 0.0 ? 0.0 : (100.0 * phase2_ms / total_ms);
    //   LOG(INFO) << "average timer(" << phase1 << "/" << phase2 << "): "
    //             << phase1_ms << "/" << phase2_ms << " ms, " << phase2_pct
    //             << "% in phase2";
    // }
