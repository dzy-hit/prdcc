...
public:
  ...
  void read_snapshot() {
    ...
    for (auto i = id; i < transactions.size(); i += context.worker_num) {
      ...
      // // timer_flag
      // const auto attempt_start = context.enable_timer
      ...
      //   transactions[i]->attemptStartTime = attempt_start;
      // }
      ...
      if (result == TransactionResult::ABORT_NORETRY) {
        ...
        //           std::chrono::steady_clock::now() -
        //           transactions[i]->attemptStartTime)
        //           .count();
  ...
  void commit_transactions() {
    // uint64_t batch_commit_count = 0; // 实验 5.3.1 统计量
    std::size_t count = 0;
    for (auto i = id; i < transactions.size(); i += context.worker_num) {
      if (transactions[i]->abort_no_retry) {
        continue;
    ...
    // // timer_flag
    // auto record_attempt_time = [this](TransactionType &txn, bool committed) {
    //   if (!context.enable_timer) {
    //     return;
    //   }
    //   const auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
    //                       std::chrono::steady_clock::now() -
    //                       txn.attemptStartTime)
    //                       .count();
    //   if (committed) {
    ...
    for (auto i = id; i < transactions.size(); i += context.worker_num) {
      ...
      if (context.aria_read_only_optmization &&
        ...
        // // timer_flag
        // record_attempt_time(*transactions[i], true);
        auto latency =
      ...
      if (transactions[i]->waw) {
        ...
        // // timer_flag
        // record_attempt_time(*transactions[i], false);
        continue;
      }
      ...
      if (context.aria_snapshot_isolation) {
        ...
        auto latency =
            std::chrono::duration_cast<std::chrono::microseconds>(
                ...
                .count();
        percentile.add(latency);
        // batch_commit_count++; // 实验 5.3.1 统计量
      } else {
    ...
    // // 实验 5.3.1 统计量
    // LOG(INFO) << "execution finished. commit transactions in batch: (" 
    //             << batch_commit_count << "/" << count << ")";
    flush_messages();
  }
