...
#include "protocol/Prodcc/AriaMessage.h"
...
public:
  ...
  ProdccExecutor(std::size_t coordinator_id, std::size_t id, DatabaseType &db,
               ...
               ProdccManager<Workload>& manager, // 接收 Manager 引用
      ...
      : Worker(coordinator_id, id), db(db), context(context),
        manager(manager), // 保存 Manager 引用
        transactions(manager.transactions), // 从 manager 获取事务向量引用
        storages(manager.storages), // 从 manager 获取 storages 引用
        epoch(epoch),
  ...
  void start() override {
    ...
    for (;;) { // Outer loop for each new Macro-Block (epoch)
      ...
      // 1. Wait for the READ phase signal
      ...
      // Wait for the manager to finish the READ phase synchronization
      ...
      // 2. Wait for PARTITION phase
      do {
        status = static_cast<ExecutorStatus>(worker_status.load());
        if (status == ExecutorStatus::EXIT) { // Check for exit again
          LOG(INFO) << "ProdccExecutor " << id << " exits.";
          return;
        }
      ...
      // --- PARTITION PHASE ---
      n_started_workers.fetch_add(1);
      manager.partition_task(this->id); // Execute the parallel partitioning task
      n_complete_workers.fetch_add(1);

      // Wait for manager to finish PARTITION phase sync
      ...
      // 2. Inner loop for executing multiple Micro-Blocks
      for (;;) { // This loop handles MB_1, MB_2, ...
        
        // Wait for the COMMIT phase signal for the next Micro-Block
        do {
          status = static_cast<ExecutorStatus>(worker_status.load());
          if (status == ExecutorStatus::Aria_READ || status == ExecutorStatus::PRODCC_PARTITION || status == ExecutorStatus::EXIT) {
            break; 
          }
        } while (status != ExecutorStatus::Aria_COMMIT);

        if (status != ExecutorStatus::Aria_COMMIT) {
            break;
        }

        // --- COMMIT PHASE (for one Micro-Block) ---
        n_started_workers.fetch_add(1);
        commit_transactions();
        n_complete_workers.fetch_add(1);

        // Wait for manager to finish this Micro-Block's sync
  ...
  void read_snapshot() {
    ...
    for (auto i = id; i < transactions.size(); i += context.worker_num) {
      ...
      // start reservation
      // 删除这里的元数据预留调用！它会在 commit_transactions 中按微块进行
  ...
  void commit_transactions() {
    // 1. 从 Manager 获取当前要执行的 Micro-Block
    const MicroBlock& current_mb = manager.get_current_micro_block();

    // 如果当前微块为空，则该 worker 在此阶段无事可做，直接返回
    if (current_mb.empty()) {
        return;
    }
    ...
    // 2. 第一阶段：对微块内的事务进行依赖分析 (Validation Phase)
    std::size_t count = 0;
    for (std::size_t i = id; i < current_mb.size(); i += context.worker_num) {
        TransactionType* txn = current_mb[i];
    ...
    for (std::size_t i = id; i < current_mb.size(); i += context.worker_num) {
        ...
        // 执行与 Aria 相同的提交逻辑来处理 WAW/WAR/RAW
        bool is_committed = false;
        bool should_abort = false;

        if (context.aria_read_only_optmization && txn->is_read_only()) {
            is_committed = true;
        } else if (txn->waw) { // waw 提前推迟
            should_abort = true;
        ...
        if (is_committed) {
            ...
            // 在Prodcc中，n_abort_lock意味着推迟的数量。我们在这里标记它，
            // 但真正的中止逻辑由Manager在下一轮Macro-Block的cleanup_batch中处理。
            protocol.abort(*txn, messages); // abort() 内部会将 txn->abort_lock 设为 true
            txn->delay_lock = true;
            n_abort_lock.fetch_add(1);
        }
